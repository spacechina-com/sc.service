...
public class ModalitiesController extends BaseController {

	@Autowired
	IModalitiesService modalitiesService;

	@RequestMapping(value = "listAll", method = RequestMethod.POST)
	public List<Pd> listAll(@RequestBody Pd pd) {
		List<Pd> data = null;
		try {
			data = modalitiesService.listAll(pd);
		} catch (Exception e) {
	...
	public Page listPage(@RequestBody Pd pd) {
		...
		List<Pd> data = null;
		try {
			data = modalitiesService.listPage(page);
		} catch (Exception e) {
			data = new ArrayList<Pd>();
			e.printStackTrace();
		}
	...
	public Pd find(@RequestBody Pd pd) {
		try {
			pd = modalitiesService.findById(pd);
		} catch (Exception e) {
			pd = null;
			e.printStackTrace();
		}
